...
let update t = function
  ...
  |Place(start, dir) -> if t.phase = 1 then let new_player = 
                                              Player.insert_ship (List.hd t.players) (Player.make_coord start) (Player.make_coord dir) t.ship in match new_player with
    |ValidB(new_player) -> if t.ship = 4 then Valid({t with phase = 2; players = List.rev (new_player::(List.nth (t.players) 1)::[])}) else if t.turn = 2 then
